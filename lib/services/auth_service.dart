...
class AuthService {
  ...
  Future<String?> signInWithFacebook() async {
    try {
      ...
      final hasNetwork = await _hasNetworkConnection();
      if (!hasNetwork) {
        print('⚠️ No network connection for Facebook Sign In');
        return '❌ Không có kết nối mạng. Vui lòng kiểm tra kết nối và thử lại.';
      }

      print('🔵 Starting Facebook Sign In...');

      // ✅ STEP 1: Trigger Facebook authentication flow
      final LoginResult result = await FacebookAuth.instance.login(
        permissions: ['email', 'public_profile'],
      );

      if (result.status == LoginStatus.cancelled) {
        print('🔴 User cancelled Facebook Sign In');
        return 'Đăng nhập bị hủy';
      }

      if (result.status != LoginStatus.success) {
        print('🔴 Facebook Sign In failed: ${result.message}');
        return 'Đăng nhập Facebook thất bại: ${result.message}';
      }
      ...
      if (accessToken == null) {
        ...
        return 'Không thể lấy thông tin đăng nhập từ Facebook';
      }

      print('🔵 Got Facebook access token');
      ...
      if (email == null || email.isEmpty) {
        print('🔴 No email from Facebook');
        await FacebookAuth.instance.logOut();
        return '❌ Không thể lấy email từ Facebook. Vui lòng cấp quyền truy cập email.';
      }

      print('🔵 Facebook user email: $email');

      // ✅ STEP 4: Check for existing users with other providers
      try {
        final existingUserQuery = await _firestore
            .collection('users')
            .where('email', isEqualTo: email)
        ...
        if (existingUserQuery.docs.isNotEmpty) {
          final existingData = existingUserQuery.docs.first.data();
          final providers = List<String>.from(existingData['providers'] ?? []);

          // Block if user exists with password or google provider only
          if ((providers.contains('password') || providers.contains('google.com')) &&
              !providers.contains('facebook.com')) {
            print('🔴 User exists with different provider - BLOCKING Facebook login');
            await FacebookAuth.instance.logOut();

            if (providers.contains('password')) {
              return '❌ Email này đã được đăng ký bằng Email/Mật khẩu.';
            } else {
              return '❌ Email này đã được đăng ký bằng Google.';
            }
          }
        }
      } catch (e) {
      ...
      await _createOrUpdateFacebookUserDocument(
        userCredential: userCredential,
        email: email,
        userData: userData,
      );

      // ✅ STEP 8: Save login state
      if (userCredential.user != null) {
        await _saveLoginState(true, userCredential.user!.uid);
      ...
      switch (e.code) {
        case 'account-exists-with-different-credential':
          return '❌ Email này đã được đăng ký bằng phương thức khác.';
        case 'invalid-credential':
          return 'Thông tin xác thực Facebook không hợp lệ.';
        case 'user-disabled':
          return 'Tài khoản này đã bị vô hiệu hóa.';
        default:
          return 'Lỗi đăng nhập Facebook: ${e.message}';
      }
    } on SocketException catch (e) {
      print('🌐 Network error: $e');
      await FacebookAuth.instance.logOut();
  ...
  /// Create or update Facebook user document in Firestore
  Future<void> _createOrUpdateFacebookUserDocument({
    ...
    try {
      print('🔵 Creating/Updating Facebook user document for: $email');
      ...
      if (docSnapshot.exists) {
        // User exists - update
        print('🔵 User exists - updating Facebook user document');
        await userDoc.update({
          'providers': ['facebook.com'], // Only Facebook provider
          ...
          'emailVerified': true, // Facebook always verifies email
          'updatedAt': FieldValue.serverTimestamp(),
        });
      } else {
        // New user - create
        print('🔵 New user - creating Facebook user document');
        await userDoc.set({
          'uid': user.uid,
          'email': email,
          'displayName': userData['name'] ?? user.displayName ?? 'User',
      ...
      print('✅ Facebook user document created/updated successfully');

      // Ensure single provider
      await _ensureSingleProvider(user.uid, 'facebook.com');

    } catch (e) {
      print('❌ Error creating/updating Facebook user document: $e');
      rethrow;
    }
  }
