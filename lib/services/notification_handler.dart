...
import 'dart:convert';
import '../main.dart'; // Import to use the global navigatorKey
import 'dart:async';

/// Global notification handler for handling notification taps
/// from any source (app foreground, background, or AlarmManager)
class NotificationHandler {
  ...
  static void handleNotificationTap(NotificationResponse response) {
    ...
    try {
      ...
      // This ensures navigation works even when app is just starting
  ...
  /// Waits for MaterialApp to be ready before navigating
  static Future<void> _navigateWithRetry(ArticleModel article, {int retryCount = 0}) async {
    ...
    if (navigatorKey.currentState != null && navigatorKey.currentContext != null) {
      // Navigator is ready, perform navigation
      try {
